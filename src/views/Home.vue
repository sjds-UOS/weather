<template>
  <div class="home"  >
    <transition name="fade" >
      <div class="mask" v-show="$store.state.menuState" @click="maskClick" style="-webkit-app-region:no-drag"></div>
    </transition>

    <side-menu></side-menu>
    <weather-nav style="-webkit-app-region:drag"></weather-nav>
    <div class="pd-lr">
      <main-board ></main-board>
      <detlpage></detlpage>
      <life-guide></life-guide>
      <!-- <day7-forecast></day7-forecast> -->
      <!-- 24小时预报 -->
      <Hour24></Hour24>
    </div>
    <!-- <page-footer></page-footer> -->
    
  </div>
</template>

<script>
import SideMenu from "@/components/content/SideMenu";
import WeatherNav from "@/components/content/WeatherNav";
import MainBoard from "@/components/content/MainBoard";
// import day7Forecast from "@/components/child/Day7";
import detlpage from "@/components/child/DetlPage";
import LifeGuide from "@/components/child/LifeGuide";
// import PageFooter from "@/components/content/PageFooter";
import Hour24 from "@/components/child/Hour24";
// import Loading from "@/components/content/Loading";

export default {
  components: {
    SideMenu,
    WeatherNav,
    MainBoard,
    detlpage,
    // day7Forecast,
    LifeGuide,
    // PageFooter,
    Hour24,
    // Loading,
  },
  methods: {
    maskClick() {
      this.$store.state.menuState = false;
    },
  },
};
</script>

<style lang="scss">
.home {
  .pd-lr {
    margin: 0 3vw;

    @media screen and (min-width: 1080px) {
      margin: 0 auto;
      width: 1014px;
    }
  }

  .nav-bar {
    position: sticky;
    left: 0;
    top: 0;
    z-index: 10;
  }

  .mask {
    position: fixed;
    top: 0;
    left: 0;
    z-index: 90;
    width: 100%;
    height: 100%;
    background-color: #0000005b;
    // backdrop-filter: saturate(180%) blur(4px);
    // transition: opacity 0.3s;
  }
}
</style>