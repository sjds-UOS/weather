<template>
  <div class="city-choose">
    <nav-bar class="nav-bar">
      <img class="back-icon" slot="left" @click="backBtn" :src="backIconUrl" style="-webkit-app-region:no-drag" />
      <div class="title" slot="center">城市列表</div>
      <img class="edit-icon" slot="right" @click="editBtn" :src="editIconUrl" v-show="!editState" v-if="editBtnHide" style="-webkit-app-region:no-drag" />
      <img class="done-icon" slot="right" @click="editBtn" :src="doneIconUrl" v-show="editState" v-if="editBtnHide" style="-webkit-app-region:no-drag" />
    </nav-bar>

    <div class="searchbar" style="-webkit-app-region:no-drag">
      <input type="text" class="search" ref="search" @focus="getFocus" placeholder="输入城市名称" v-model.trim="keyword" />
      <button v-show="isShow" @click="cancelBtn" style="-webkit-app-region:no-drag" >取消</button>
    </div>

    <div class="selected-city" v-show="lists == ''">
      <ul>
        <li v-if="this.$store.state.city" @click="jumpTo($store.state.cityid)" style="-webkit-app-region:no-drag">
        ...
        <li v-for="(city, index) in $store.state.selectedCitys" :key="index" @click="jumpTo(city.id)" style="-webkit-app-region:no-drag">
          {{ city.name }}
          <div class="delBtn" @click.stop="delBtn(index)" v-show="editState" style="-webkit-app-region:no-drag" >
            删除
          </div>
        </li>
      </ul>
    ...
    <div class="city-area" v-show="isShow" ref="resultshow">
      <ul>
        <li v-for="item of lists" :key="item.id" @click="addNewCity(item, item.id)" style="-webkit-app-region:no-drag">
          <span>{{ item.name }}</span>
